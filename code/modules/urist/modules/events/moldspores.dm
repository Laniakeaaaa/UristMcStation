...
#define MINITAG_MOLDMAN 0

/datum/event/moldmanspores
	announceWhen = 10
    oneshot = 1
	endWhen = 11
	var/area/location
...
/datum/event/moldmanspores/start()
	...
	if(!vermin_turfs)
		log_debug("Unable to find a suitable area to spawn moldmen, aborting...")
		kill()
	...
	spawn(0)
		...
		log_and_message_admins("Attnetion: Moldmen spawned in \the [location]", location = pick_area_turf(location))
...
/datum/event/moldmanspores/announce()
